<div class="homebg1 text-light" id="home">
	...
	<div class="container mt-3">
		<h3 class="text-center mt-3"><strong>EDIT USER</strong></h3>
		<?php

		if (isset($_POST['submit'])) {
			$drvnum = $_POST['nomor'];
			$drvname1 = $_POST['nama_depan'];
			$drvname2 = $_POST['nama_belakang'];
			$drvstatus = $_POST['status'];
			$drvteam = $_POST['nama_tim'];
			$teambike = $_POST['motorbalap'];
			$fotoid = $_POST['id_foto'];

			if (isset($_POST['id'])) { // ini adalah proses edit

				$id = $_POST['id'];
				$sql = 	"UPDATE pembalapmotogp SET nomor='$drvnum', nama_depan='$drvname1', nama_belakang='$drvname2', status='$drvstatus', 
		id_timmotogp='$drvteam', id_motorbalap='$teambike', id_fotopembalap='$fotoid' WHERE id='$id'";


				if ($conn->query($sql)) {
					echo "<p>Proses edit berhasil.</p>";
				} else {
					echo "<p>Proses edit gagal.</p>";
					echo "Error: " . $sql . "<br>" . $conn->error;
				}
				echo "<a href='index.php?page=rider' class='btn btn-primary'>Home</a>";
			}
		} else {
		...
		?>
			...
			<form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post">
				...
				<a href='index.php?page=public-db' class='btn btn-danger mb-3'>Back</a>
			</form>
